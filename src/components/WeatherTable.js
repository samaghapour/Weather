...
const WeatherTable = () => {
    ...
    weatherContext.Data.forEach(item => {
    ...
    return (
        ...
        <div className="table">
            ...
            {tableData.map(item =>{
                        return (
                                <tr key={item.key}>
                                    <th className='date'>{item.date} <span>{item.description}</span></th>
                                    <th className="temp"> <img src={item.icon} alt="icon" style={{width:'50px' ,height:'50px'}}/> {item.temp}</th>
                                </tr>
                            )
            })}
            </thead>
